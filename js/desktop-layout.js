...
@media(min-width:1000px){
  ...
  .hero{grid-template-columns:86px minmax(280px,1fr) auto;gap:18px;padding:18px 22px}
  .hero>.status-orb{width:78px;height:78px}
  .health-value{font-size:46px}.health-text{font-size:18px}.coverage{font-size:12px}
  .hero>#oneTouchBtn{min-width:230px;padding:14px 22px;font-size:14px}
  ...
  .desktop-overview-card{background:#0e1728;border:1px solid var(--line);border-radius:13px;padding:10px 12px;min-width:0}
  .desktop-overview-card .label{font-size:10px;letter-spacing:.05em;text-transform:uppercase;color:var(--muted);margin-bottom:4px}
  .desktop-overview-card .value{font-size:16px;font-weight:800;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
  .desktop-overview-card .sub{font-size:10px;color:var(--muted);margin-top:2px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
...
function ensureOverview(){let host=$('#kcDesktopOverview');const main=$('main');const tabs=$('.tabs');if(!main||!tabs)return;if(!host){host=document.createElement('section');host.id='kcDesktopOverview';host.className='desktop-overview';tabs.after(host)}return host}
function renderOverview(){if(!matchMedia('(min-width:1000px)').matches)return;const host=ensureOverview();if(!host)return;const mode=text('#runtimeMode','—'),last=text('#lastRunLabel','Noch kein Lauf'),coverage=text('#coverageText','—'),core=text('#metricCore','—'),future=text('#metricFuture','—'),mirror=text('#metricMirror','—');host.innerHTML=`
...
<div class="desktop-overview-card"><div class="label">Academy / Mirror</div><div class="value">${future}</div><div class="sub">Spiegelung ${mirror}</div></div>`}
function refresh(){installStyles();if(matchMedia('(min-width:1000px)').matches)renderOverview();else $('#kcDesktopOverview')?.remove()}
