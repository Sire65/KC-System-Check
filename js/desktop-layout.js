...
function renderHeroExtra(){const host=ensureHeroExtra();if(!host)return;const gh=findSystemCard('GitHub'),mir=findSystemCard('Spiegelung');const mirrorDetail=cardDetail(mir).replace(/^0 Abweichungen\s*·\s*/i,'');host.innerHTML=`<div class="desktop-hero-extra-row"><span class="k">GitHub</span><span class="v">${cardValue(gh)}</span></div><div class="desktop-hero-extra-row"><span class="k">Spiegelung</span><span class="v">${cardValue(mir)}${mirrorDetail?` · ${mirrorDetail}`:''}</span></div>`}
...
<div class="desktop-overview-card"><div class="label">Academy / Mirror</div><div class="value">${future}</div><div class="sub">Spiegelung ${mirror}</div></div>`;renderHeroExtra()}
