...
function ensureHost(){
  ...
  let host=document.querySelector("#operationsOverview");if(host)return host;
  const article=document.createElement("article");article.className="card";article.id="operationsOverviewCard";
  ...
  if(!document.querySelector("#kcOpsOverviewStyle")){const style=document.createElement("style");style.id="kcOpsOverviewStyle";style.textContent='.kc-ops-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:9px}.kc-ops-card{padding:12px;border:1px solid var(--line);border-radius:14px;background:#0e1728;min-width:0}.kc-ops-head{display:flex;justify-content:space-between;gap:8px;align-items:center}.kc-ops-tag{font-size:10px;font-weight:800;padding:4px 7px;border:1px solid var(--line);border-radius:999px;white-space:nowrap}.kc-ops-card strong{display:block;overflow-wrap:anywhere}.kc-ops-card .dot{margin-right:7px}.kc-ops-facts{display:grid;gap:4px;margin-top:9px;padding-top:8px;border-top:1px solid var(--line)}.kc-ops-fact{display:flex;justify-content:space-between;gap:10px;font-size:11px}.kc-ops-ready{font-weight:850;letter-spacing:.02em}@media(max-width:620px){.kc-ops-grid{grid-template-columns:1fr}.kc-ops-fact{display:block}}';document.head.appendChild(style)}
