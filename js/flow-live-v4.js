...
export function trafficView(entry,now=Date.now()){
  if(!entry?.at)return{moving:false,recent:false,bad:false,cls:"idle",tag:"KEIN VERKEHR"};
  const age=Math.max(0,now-entry.at);
  const bad=!!entry.bad&&age<=ERROR_MS;
  const hasTraffic=Number(entry.events||0)>0||Number(entry.bytes||0)>0;
  const moving=!bad&&hasTraffic&&age<=LIVE_MS;
  const recent=!bad&&hasTraffic&&!moving&&age<=RECENT_TRAFFIC_MS;
  return{moving,recent,bad,cls:bad?"bad":moving?"active":"idle",tag:bad?"STÖRUNG":moving?"VERKEHR":recent?"KÜRZLICH":"RUHE",age};
}
...
function rememberSeen(id,at){
  seen.set(id,at);
  if(seen.size>1200){const cut=Date.now()-RETAIN_MS;for(const[k,t]of seen)if(t<cut)seen.delete(k)}
}
function touch(from,to,{at=Date.now(),events=1,bytes=0,bad=false,source="Messung"}={}){
  if(!from||!to||from===to)return;
  const key=routeKey(from,to);
  if(!ROUTE_BY_KEY.has(key))return;
...
function ingestFlowRows(live){
  ...
  for(const f of live?.flows||[]){
    ...
    const from=normalizeFlowNode(f.source_id||f.program_id),to=normalizeFlowNode(f.target_id);
    if(!from||!to||!ROUTE_BY_KEY.has(routeKey(from,to)))continue;
    let at=parseTime(f.measured_at||f.received_at);
    if(from==="pc-backup"&&backupAt)at=backupAt;
    if(!at)continue;
    const id=`flow|${from}|${to}|${at}|${f.event_count||0}|${f.byte_count||0}|${f.status||""}`;
    if(seen.has(id))continue;
    rememberSeen(id,at);
    const events=Math.max(0,Number(f.event_count||0)),bytes=Math.max(0,Number(f.byte_count||0));
    if(events>0||bytes>0||isBad(f.status))touch(from,to,{at,events:events||1,bytes,bad:isBad(f.status),source:f.flow_type||"Flow-Telemetrie"});
  }
  if(backup&&backupAt){
    const to=backupTarget(backup);
    if(to){
      const id=`backup|${to}|${backupAt}`;
      if(!seen.has(id)){
        rememberSeen(id,backupAt);
        touch("pc-backup",to,{at:backupAt,events:1,bytes:Number(backup.last_backup_stored_bytes||backup.last_backup_bytes||0),bad:isBad(backup.last_backup_status||backup.status),source:"Backup-Abschluss"});
      }
    }
  }
}
...
function ingestHeartbeatCounters(live){
  for(const h of live?.heartbeats||[]){
    const from=normalizeFlowNode(h.source_id||h.program_id);
    if(!from||from==="supabase"||!ROUTE_BY_KEY.has(routeKey(from,"supabase")))continue;
    const tx=Number(h.traffic_tx),at=parseTime(h.measured_at||h.received_at);
    if(!Number.isFinite(tx)||tx<0||!at)continue;
    ...
    const delta=counterDelta(prev.value,tx);
    counters.set(id,{value:tx,at});
...
function ingestBroadcast(p){
  const from=normalizeFlowNode(p?.von);
  if(!from||!Array.isArray(p?.kanten))return;
  const at=parseTime(p.zeit)||Date.now();
  for(const e of p.kanten){
    const to=normalizeFlowNode(e?.nach),events=Math.max(0,Number(e?.req||0)),bytes=Math.max(0,Number(e?.bytes||0)),bad=Number(e?.fehler||0)>0||isBad(p?.status);
    if(to&&(events>0||bytes>0||bad))touch(from,to,{at,events:events||1,bytes,bad,source:"Realtime"});
  }
  renderAll();
}
...
function overall(list){return list.some(x=>x.view.bad)?"bad":list.some(x=>x.view.moving)?"live":list.some(x=>x.view.recent)?"recent":"idle"}

function ensureCss(){
  if(typeof document==="undefined"||document.querySelector("#kcFlowTruthCssV4"))return;
  document.querySelector("#kcFlowTruthCss")?.remove();
  const s=document.createElement("style");s.id="kcFlowTruthCssV4";s.textContent=`
#kcdfKarteBadge{display:none!important}.kc-truth-list{display:grid;gap:8px;margin-top:10px}.kc-truth-row{display:grid;grid-template-columns:minmax(82px,1fr) minmax(70px,1.2fr) minmax(82px,1fr) auto;gap:8px;align-items:center;padding:10px;border:1px solid var(--line);border-radius:13px;background:#0e1728}.kc-truth-node{font-size:12px;font-weight:800;overflow:hidden;text-overflow:ellipsis}.kc-truth-node.to{text-align:right}.kc-truth-track{position:relative}.kc-truth-line{height:5px;border-radius:999px;background:#30394a;position:relative;overflow:hidden}.kc-truth-line.active{background:var(--ok)}.kc-truth-line.bad{background:var(--bad)}.kc-truth-line.active:after{content:"";position:absolute;width:10px;height:10px;border-radius:50%;top:50%;left:-10px;transform:translateY(-50%);background:var(--ok);box-shadow:0 0 10px var(--ok);animation:kcTruthMove 1.35s linear infinite}.kc-truth-recent-dot{position:absolute;width:7px;height:7px;border-radius:50%;background:var(--ok);right:2px;top:50%;transform:translateY(-50%);box-shadow:0 0 5px var(--ok)}.kc-truth-tag{font-size:9px;font-weight:900;padding:4px 6px;border:1px solid var(--line);border-radius:999px;white-space:nowrap}.kc-truth-meta{grid-column:1/-1;color:var(--muted);font-size:11px;margin-top:-3px}.kc-truth-map svg{display:block;width:100%;height:auto;max-width:900px;margin:0 auto}.kc-truth-legend{display:flex;gap:14px;flex-wrap:wrap;color:var(--muted);font-size:12px;margin-top:8px}.kc-truth-swatch{display:inline-block;width:20px;height:3px;border-radius:2px;background:#30394a;margin-right:5px;vertical-align:middle}.kc-truth-swatch.live{background:var(--ok)}.kc-truth-mini{display:inline-block;width:7px;height:7px;border-radius:50%;background:var(--ok);margin:0 5px;vertical-align:middle}@keyframes kcTruthMove{to{left:100%}}@media(max-width:620px){.kc-truth-row{grid-template-columns:minmax(68px,1fr) 50px minmax(68px,1fr) auto;padding:9px 7px}.kc-truth-node{font-size:11px}}@media(prefers-reduced-motion:reduce){.kc-truth-line.active:after{animation:none;left:calc(50% - 5px)}}`;
  document.head.appendChild(s);
}

function renderList(){
  if(typeof document==="undefined")return;
  const host=document.querySelector("#kcLiveFlowOverview");if(!host)return;
  ensureCss();host.dataset.kcFlowTruth="4";
  const list=entries(),o=overall(list),badge=o==="bad"?"STÖRUNG":o==="live"?"LIVE":o==="recent"?"KÜRZLICH AKTIV":"RUHE";
  host.innerHTML=`<div class="row between"><div><div class="eyebrow">Echte Betriebsdaten</div><h3>Leitflüsse</h3><div class="muted small">Grün + wandernder Punkt = echter Verkehr ≤ 60 s. Graue Linie + kleiner grüner Punkt = echter Verkehr innerhalb der letzten 15 min.</div></div><span class="badge${o==="live"?" live":""}">${badge}</span></div><div class="kc-truth-list">${list.map(x=>{const st=x.state,v=x.view,meta=st?.at?[x.kind,st.events?`${st.events} Ereignis${st.events===1?"":"se"}`:"",bytesText(st.bytes),`letzter echter Verkehr ${ago(v.age)}`,st.source].filter(Boolean).join(" · "):`${x.kind} · bisher kein gemessener Verkehr`;return`<div class="kc-truth-row"><div class="kc-truth-node">${esc(x.fromLabel)}</div><div class="kc-truth-track"><div class="kc-truth-line ${v.cls}"></div>${v.recent?'<span class="kc-truth-recent-dot" title="Echter Verkehr innerhalb der letzten 15 Minuten"></span>':''}</div><div class="kc-truth-node to">${esc(x.toLabel)}</div><span class="kc-truth-tag">${esc(v.tag)}</span><div class="kc-truth-meta">${esc(meta)}</div></div>`}).join("")}</div>`;
}

const MAP_NODES={
  "kasse-01":{x:95,y:40,label:"Kasse 01"},"kasse-02":{x:95,y:92,label:"Kasse 02"},"pc-manager":{x:95,y:144,label:"PC Manager"},"money-butler":{x:95,y:196,label:"Money Butler"},"kc-verwaltung":{x:95,y:248,label:"KC Verwaltung"},"dp-app":{x:95,y:300,label:"KC Dienstplan"},"pc-backup":{x:95,y:365,label:"PC Backup"},
  supabase:{x:390,y:195,label:"Supabase · KC Core"},"neon-mirror":{x:675,y:85,label:"Neon · Spiegel"},"neon-vault":{x:675,y:235,label:"Neon · Backup"},b2:{x:675,y:365,label:"Backblaze B2"}
};
...
function renderMap(){
  ...
  host.innerHTML=`<div class="kc-truth-map"><div class="row between" style="margin-bottom:5px"><div class="muted small">Keine Dekoration: Bewegung entsteht nur durch echten gemessenen Verkehr.</div><span class="badge${o==="live"?" live":""}">${o==="bad"?"STÖRUNG":o==="live"?"VERKEHR":o==="recent"?"KÜRZLICH":"RUHE"}</span></div><svg viewBox="0 0 780 420" role="img" aria-label="KC Datenfluss: grau ohne aktuellen Verkehr, statischer grüner Punkt bei kürzlichem Verkehr, grün bewegt bei aktuellem Verkehr">${paths}${nodes}</svg><div class="kc-truth-legend"><span><i class="kc-truth-swatch"></i>kein aktueller Verkehr</span><span><i class="kc-truth-swatch"></i><i class="kc-truth-mini"></i>echter Verkehr ≤ 15 min</span><span><i class="kc-truth-swatch live"></i>bewegter Punkt = echter Verkehr ≤ 60 s</span></div></div>`;
}
...
function ensureRealtime(runtime){
  ...
  const base=String(runtime?.apiBaseUrl||""),m=base.match(/^https:\/\/([a-z0-9-]+\.supabase\.co)/i),token=runtime?.apiToken||"",key=`${m?.[1]||""}|${token}`;
  if(!m||!token)return;if(key===wsRuntimeKey&&ws)return;
  realtimeStop();wsRuntimeKey=key;let ref=0,closed=false;
  const send=(topic,event,payload)=>{if(ws?.readyState===1)ws.send(JSON.stringify({topic,event,payload,ref:String(++ref)}))};
  const connect=()=>{if(closed)return;try{ws=new WebSocket(`wss://${m[1]}/realtime/v1/websocket?apikey=${encodeURIComponent(token)}&vsn=1.0.0`)}catch{return}ws.onopen=()=>{send(`realtime:${KANAL}`,"phx_join",{config:{broadcast:{self:false},presence:{key:""},postgres_changes:[]},access_token:token});wsHeartbeat=setInterval(()=>send("phoenix","heartbeat",{}),30_000)};ws.onmessage=e=>{let msg;try{msg=JSON.parse(e.data)}catch{return}if(msg.event==="phx_reply"&&msg.topic===`realtime:${KANAL}`)realtimeState=msg.payload?.status==="ok"?"verbunden":"abgelehnt";if(msg.event==="broadcast"&&msg.payload?.event==="fluss")ingestBroadcast(msg.payload.payload)};ws.onclose=()=>{clearInterval(wsHeartbeat);wsHeartbeat=null;realtimeState="getrennt";if(!closed)wsTimer=setTimeout(connect,5_000)};ws.onerror=()=>{}};
  connect();return()=>{closed=true;realtimeStop()};
}
...
export const __flowTruthForTests={STATIC_ROUTES,activity,counters,get realtimeState(){return realtimeState},ingest,entries,LIVE_MS,RECENT_TRAFFIC_MS};
