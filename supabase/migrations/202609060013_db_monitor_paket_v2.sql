...
as $$
  with objects as (
    ...
    select o.relname || case when o.relkind = 'm' then ' (materialisiert)' else ' (View mit Eigentümerrechten)' end as name
    from objects o
...
begin
  ...
  if jsonb_array_length(c -> 'sequences_near_limit') > 0 then
    kritisch := kritisch || format('%s Sequenz(en) über 70 %% ausgeschöpft', jsonb_array_length(c -> 'sequences_near_limit')); end if;
  ...
  if jsonb_array_length(s -> 'permissive_policies') > 0 then
    warnungen := warnungen || format('%s Policy(s) mit uneingeschränktem Zugriff', jsonb_array_length(s -> 'permissive_policies')); end if;
  ...
  if jsonb_array_length(c -> 'vacuum_backlog') > 0 then
    warnungen := warnungen || format('%s Tabelle(n) mit echtem Vacuum-Rückstand', jsonb_array_length(c -> 'vacuum_backlog')); end if;
  if jsonb_array_length(schwerer_leerraum) > 0 then
    warnungen := warnungen || format('%s große Tabelle(n) mit über 50 %% Leerraum', jsonb_array_length(schwerer_leerraum)); end if;
  ...
  if jsonb_array_length(s -> 'tables_without_rls') > 0 and jsonb_array_length(s -> 'client_roles_present') = 0 then
    hinweise := hinweise || format('%s Tabelle(n) ohne RLS, aber keine der geprüften Client-Rollen existiert – kein Zugriff aus dem Netz möglich',
