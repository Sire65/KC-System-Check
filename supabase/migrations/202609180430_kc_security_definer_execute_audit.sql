-- KC System Check wrapper for the portable SECURITY DEFINER execute audit.
-- The portable implementation stays in share/db-monitor/security-definer-execute.sql;
-- this migration exposes only a service-role RPC for the Edge Function.
...
create or replace function public.kc_system_check_security_definer_execute_audit()
returns jsonb
language sql
stable
security definer
set search_path = pg_catalog
as $$
  select db_monitor.security_definer_execute_audit('public', array['anon','authenticated']);
$$;

revoke all on function public.kc_system_check_security_definer_execute_audit() from public;
do $$ begin
  if exists(select 1 from pg_roles where rolname='anon') then
    revoke all on function public.kc_system_check_security_definer_execute_audit() from anon;
  end if;
  if exists(select 1 from pg_roles where rolname='authenticated') then
    revoke all on function public.kc_system_check_security_definer_execute_audit() from authenticated;
  end if;
  if exists(select 1 from pg_roles where rolname='service_role') then
    grant execute on function public.kc_system_check_security_definer_execute_audit() to service_role;
  end if;
end $$;
