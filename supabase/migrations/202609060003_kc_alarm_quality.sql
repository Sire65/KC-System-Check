-- Serverseitiges Gegenstueck zu js/alarm-policy.js: Entprellung,
-- Abhaengigkeitsunterdrueckung, Wartungsfenster und Wiedervorlage.
-- Die Alarmierungsfunktion ruft kc_system_check_alarm_apply() auf und
-- verschickt nur, was in 'notify' zurueckkommt.

create table if not exists public.kc_system_check_alarm_state (
...
comment on table public.kc_system_check_alarm_state is
  'Gedaechtnis der Alarmbewertung. Ohne diesen Zustand gibt es keine Entprellung und jeder Messfehler wird zum Alarm.';

alter table public.kc_system_check_alarm_state enable row level security;
revoke all on table public.kc_system_check_alarm_state from anon, authenticated;

drop policy if exists kc_system_check_alarm_state_deny_anon on public.kc_system_check_alarm_state;
create policy kc_system_check_alarm_state_deny_anon on public.kc_system_check_alarm_state
...
begin
  ...
  return jsonb_build_object('checked_at', v_now, 'alarms', v_alarms, 'suppressed', v_suppressed, 'notify', v_notify);
end;
$$;
...
-- Totmannschalter: wie alt ist der juengste protokollierte Pruflauf?
create or replace function public.kc_system_check_watchdog(p_max_age_minutes integer default 1500)
returns jsonb
language sql
security definer
set search_path = pg_catalog, public
as $$
  select jsonb_build_object(
    'checked_at', now(),
    'last_run_at', (select max(checked_at) from public.kc_system_check_history),
    'age_minutes', (select (extract(epoch from now() - max(checked_at)) / 60)::int from public.kc_system_check_history),
    'max_age_minutes', p_max_age_minutes,
    'ok', coalesce((select (extract(epoch from now() - max(checked_at)) / 60)::int from public.kc_system_check_history) <= p_max_age_minutes, false)
  );
$$;

revoke all on function public.kc_system_check_watchdog(integer) from public, anon, authenticated;
grant execute on function public.kc_system_check_watchdog(integer) to service_role;
