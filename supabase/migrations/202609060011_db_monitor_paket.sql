-- Eine Pruefung, ein Ort: das tragbare Paket db_monitor.
--
-- Die Sicherheits- und die Kapazitaetspruefung standen bisher als langer
-- SQL-Text direkt in den KC-Funktionen. Dieselbe Logik wird jetzt als
-- eigenstaendiges Paket gefuehrt (share/db-monitor/install.sql), das auf jeder
-- PostgreSQL-Datenbank laeuft - Supabase, Neon, eigener Server. Die KC-
-- Funktionen rufen es nur noch auf.
--
-- Der Abschnitt bis zur Trennlinie ist Zeichen fuer Zeichen der Inhalt von
-- share/db-monitor/install.sql. Ein Test wacht darueber, dass die beiden nicht
-- auseinanderlaufen (tests/sql/db-monitor.test.sql).
...
-- =============================================================================
-- KC-Anschluss: die beiden Pruefungen rufen ab hier nur noch das Paket auf.
-- =============================================================================
-- Vorher stand dieselbe Logik zweimal da - einmal in kc_system_check_security_audit,
-- einmal in kc_system_check_db_capacity. Ein Fehler musste also zweimal gefunden
-- und zweimal behoben werden. Genau das ist beim Klammerfehler unten passiert.
--
-- Die Namen der Felder bleiben, damit die Edge Function unveraendert weiterlaeuft.
-- Einziger Unterschied im Inhalt: 'uncovered_grants' heisst hier weiter
-- 'public_grants', und der Bericht liefert zusaetzlich 'client_roles_present'.
...
set search_path = pg_catalog, public
as $kc$
  -- Behobener Klammerfehler: vorher stand hier
  --   where (relkind = 'v' and not invoker) or relkind = 'm' and exists (...)
  -- and bindet staerker als or, also galt die Rechte-Bedingung nur fuer
  -- materialisierte Views. Eine Definer-View, die niemand erreichen kann, wurde
  -- dadurch als Loch gemeldet. Im Paket steht die Klammer richtig.
  select (v - 'uncovered_grants') || jsonb_build_object('public_grants', v -> 'uncovered_grants')
  from db_monitor.security_audit('public', array['anon','authenticated']) v;
$kc$;
...
set search_path = pg_catalog, public
as $kc$
  select db_monitor.capacity('public');
$kc$;
