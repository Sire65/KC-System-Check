-- Storage target telemetry for PC Backup Vault / KC System Check.
-- Only sanitized operational metadata is stored here. No credentials, usernames,
-- local/UNC paths, remote roots, DSNs, tokens, recovery keys or file names.

alter table if exists public.kc_backup_machine_telemetry
  add column if not exists storage_targets jsonb not null default '[]'::jsonb;

alter table if exists public.kicc_backup_telemetry
  add column if not exists storage_targets jsonb not null default '[]'::jsonb;

comment on column public.kc_backup_machine_telemetry.storage_targets is
  'Sanitized read-only target health: nas_backup, hidrive_1, hidrive_2. No paths or credentials.';

comment on column public.kicc_backup_telemetry.storage_targets is
  'Sanitized read-only target health: nas_backup, hidrive_1, hidrive_2. No paths or credentials.';
