...
let activeTrace:any={};
const traced=(headers:any={})=>withTraceHeaders(headers,activeTrace);
async function timed(url:string,init:RequestInit={}){const s=Date.now(),r=await fetch(url,{...init,headers:traced(init.headers||{}),cache:"no-store"});return{r,ms:Date.now()-s}}
