...
let b2=endpointResult("b2","Backblaze B2","storage",b2Direct,"Adapter bereit; noch keine B2-Telemetrie oder sichere Read-only-Verbindung");const b2Target=Array.isArray(tele?.storage_targets)?tele.storage_targets.find((x:any)=>x?.id==="b2_backup"):null;if(!b2Direct&&b2Target){const t=telemetryState(b2Target.status);b2={...b2,status:t.status,health:t.health,latency:Number.isFinite(Number(b2Target.latencyMs))?Number(b2Target.latencyMs):null,capacityLabel:tele.last_backup_bytes?`${mb(Number(tele.last_backup_bytes))} MB Original${tele.last_backup_stored_bytes!==null&&tele.last_backup_stored_bytes!==undefined?` · ${mb(Number(tele.last_backup_stored_bytes))} MB gespeichert`:""}`:"PC Backup Vault B2-Prüfung",detail:`${b2Target.detail||"B2-Ziel geprüft"}${b2Target.checkedAt?` · geprüft ${b2Target.checkedAt}`:""} · Backup Vault wird manuell gestartet`,metrics:{direct_check:false,telemetry:true,target_health:true,measured_at:tele.measured_at,target_checked_at:b2Target.checkedAt||null,last_backup_at:tele.last_backup_at,last_backup_status:tele.last_backup_status,last_backup_bytes:tele.last_backup_bytes,last_backup_stored_bytes:tele.last_backup_stored_bytes,storage_target:tele.storage_target}}}else if(!b2Direct&&tele?.b2_status){const t=telemetryState(tele.b2_status);b2={...b2,status:t.status,health:t.health,capacityLabel:tele.last_backup_bytes?`${mb(Number(tele.last_backup_bytes))} MB Original${tele.last_backup_stored_bytes!==null&&tele.last_backup_stored_bytes!==undefined?` · ${mb(Number(tele.last_backup_stored_bytes))} MB gespeichert`:""}`:"PC Backup Vault Telemetrie",detail:`B2 ${tele.b2_status} · letzter Backup-Status ${tele.last_backup_status||"unbekannt"}${tele.integrity?` · Integrität ${tele.integrity}`:""}${tele.restore?` · Restore-Test ${tele.restore}`:""}${tele.app_version?` · App v${tele.app_version}`:""} · Backup Vault wird manuell gestartet`,metrics:{direct_check:false,telemetry:true,measured_at:tele.measured_at,last_backup_at:tele.last_backup_at,last_backup_status:tele.last_backup_status,last_backup_bytes:tele.last_backup_bytes,last_backup_stored_bytes:tele.last_backup_stored_bytes,storage_target:tele.storage_target}}}
