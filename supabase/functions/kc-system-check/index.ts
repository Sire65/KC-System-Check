...
function mirrorDetail(mh:any,snap:any,snapFehlt:boolean){
  ...
  if(mh.status==="healthy")teile.push(`${snap?.mirror?.mismatch_count??0} Abweichungen · letzter Lauf vor ${mh.age_min??"?"} min`);
  else if(issue?.tabelle||issue?.message){
    // Tabelle und Meldung des betroffenen Laufs, nicht bloss "Warnhinweis".
    teile.push(`${issue.tabelle||"unbenannte Tabelle"}: ${issue.message||issue.status}`);
