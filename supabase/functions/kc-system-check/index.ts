...
import { withTraceHeaders } from "../../../js/trace-context.js";
const CORS={"Access-Control-Allow-Origin":"*","Access-Control-Allow-Headers":"authorization, x-client-info, apikey, content-type, traceparent, tracestate, baggage","Access-Control-Allow-Methods":"GET, OPTIONS","Content-Type":"application/json; charset=utf-8","Cache-Control":"no-store"};
...
async function timed(url:string,init:RequestInit={},trace:any={}){const s=Date.now(),r=await fetch(url,{...init,headers:withTraceHeaders(init.headers||{},trace),cache:"no-store"});return{r,ms:Date.now()-s}}
...
async function leitstand(url:string){const k=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!,x=await timed(`${url}/rest/v1/rpc/kc_system_leitstand_snapshot`,{method:"POST",headers:{apikey:k,Authorization:`Bearer ${k}`,"Content-Type":"application/json"},body:"{}"});if(!x.r.ok)throw new Error(`leitstand_http_${x.r.status}`);const data=await x.r.json();return{version:"0.4.1-live",...data,server_latency_ms:x.ms,thresholds:{heartbeat_warn_seconds:90,heartbeat_critical_seconds:180},refresh_seconds:30}}
...
async function cloudflareEdge(own:string,service:string,trace:any={}){
  try{
    const x=await timed(`${own}/functions/v1/kc-edge-health`,{headers:{apikey:service,Authorization:`Bearer ${service}`}},trace);
    let d:any=null;try{d=await x.r.json()}catch{}
    if(!x.r.ok)return{id:"kc_edge",name:"Cloudflare / Edge",kind:"edge",status:"warning",health:70,latency:x.ms,usage:null,capacityLabel:"Read-only-Status auffällig",detail:`Cloudflare Edge-Prüfung HTTP ${x.r.status}`,metrics:{direct_check:true,http_status:x.r.status}};
    const st=d?.status==="healthy"?"healthy":d?.status==="warning"?"warning":d?.status==="critical"?"critical":"not_configured";
    const health=st==="healthy"?100:st==="warning"?75:st==="critical"?35:null;
    return{id:"kc_edge",name:"Cloudflare / Edge",kind:"edge",status:st,health,latency:Number.isFinite(Number(d?.latencyMs))?Number(d.latencyMs):x.ms,usage:null,capacityLabel:st==="not_configured"?"Vorbereitet · Zugang fehlt":"Worker · Hyperdrive · KV",detail:d?.detail||"Cloudflare Edge read-only geprüft",metrics:{direct_check:true,worker:d?.worker??null,hyperdrive:d?.hyperdrive??null,kv:d?.kv??null,checked_at:d?.checkedAt??null}};
  }catch{return{id:"kc_edge",name:"Cloudflare / Edge",kind:"edge",status:"warning",health:70,latency:null,usage:null,capacityLabel:"Read-only-Status auffällig",detail:"Cloudflare Edge-Prüfung nicht erreichbar",metrics:{direct_check:true,http_status:0}}}
}
...
function performanceResult(res:any){
  if(!res.ok)return notDeployed("db_performance","Datenbank-Performance","performance",res.status);
  const d=res.data||{},blocked=Number(d.blocked_sessions||0),waiters=Number(d.lock_waiters||0),long=Number(d.long_running_queries||0),active=Number(d.active_sessions||0),longest=Number(d.longest_active_ms||0);
  const status=blocked>0||waiters>0?"critical":long>0?"warning":"healthy";
  const parts=[`${active} aktive Session(s)`];
  if(blocked)parts.push(`${blocked} blockierte Session(s)`);
  if(waiters)parts.push(`${waiters} Lock-Wartende`);
  if(long)parts.push(`${long} Abfrage(n) länger als 30 s`);
  if(longest>0)parts.push(`längste aktive Abfrage ${Math.round(longest/100)/10} s`);
  return{id:"db_performance",name:"Datenbank-Performance",kind:"performance",status,health:status==="critical"?35:status==="warning"?72:100,latency:res.ms,usage:null,capacityLabel:blocked||waiters?`${blocked} blockiert · ${waiters} Lock-Wartende`:long?`${long} Langläufer`:"Keine Blockierung",detail:parts.join(" · "),metrics:{active_sessions:active,blocked_sessions:blocked,lock_waiters:waiters,long_running_queries:long,longest_active_ms:longest,threshold_ms:Number(d.threshold_ms||30000),query_text_exposed:false,measured_at:d.measured_at||null}};
}
...
function programResult(res:any){
  ...
  const metrics={total:gesamt,connected:angebunden,watched:ueberwacht,reported_24h:Number(d.gemeldet_24h||0),without_heartbeat:ohne,overdue:ueberfaellig,reporting_trouble:stoerung,recent:d.zuletzt||[],unregistered:d.nicht_registriert||[]};
  const teile:string[]=[];
...
function neonBudgetResult(res:any){
  const id="neon_compute_budget",name="Neon · Compute-Budget",kind="capacity";
...
select jsonb_build_object(
  ...
  'haengende_saetze', (select count(*) from public.kc_backup_sets where status='running' and started_at < now() - interval '2 hours' and started_at > now() - interval '48 hours'),
  'altlasten', (select count(*) from public.kc_backup_sets where status in ('running','error') and started_at < now() - interval '48 hours'),
...
function backupResult(res:any){
  ...
  return{id,name,kind,status,health:status==="critical"?35:status==="warning"?72:100,latency:res.ms,usage:null,capacityLabel:alter===null?"Keine Sicherung":`vor ${alter} h · ${mb(Number(d.bytes||0))} MB`,detail:teile.join(" · "),metrics:{last_ok_at:d.letzter_ok_am,age_hours:alter,tables:d.tabellen,tables_ok:d.tabellen_ok,rows:d.zeilen,bytes:d.bytes,verification_status:d.pruefung_status,verification_age_hours:pAlter,verification_tables:d.pruefung_tabellen,verification_failed:fehler,stuck_runs:haengend,stale_sets:altlasten,sets_8d:d.saetze_8_tage,newest_status:d.neuester_status}};
}
...
// PC Backup Vault ist eine manuell gestartete Anwendung. Das Alter eines
// erfolgreichen Laufs ist deshalb nur Information und darf die Ampel nicht
// faerben. Gelb/Rot kommt nur aus einem tatsaechlich gemeldeten Warn-/Fehlerstatus.
function backupAgeRules(b2:any,tele:any,masch:any){
  ...
  if(tele){
    const infos:string[]=[];
    if(bAge!==null)infos.push(`letzter manueller Backup-Lauf vor ${tageText(bAge)}`);
    if(tAge!==null)infos.push(`letzte Telemetrie vor ${tageText(tAge)}`);
    infos.push("kein täglicher Lauf erwartet");
    return{...b2,detail:`${b2.detail}${infos.length?` · ${infos.join(" · ")}`:""}`,metrics:{...b2.metrics,backup_age_hours:bAge===null?null:Math.round(bAge),telemetry_age_hours:tAge===null?null:Math.round(tAge),machine_last_seen_at:masch?.last_seen_at??null,machine_paired:!!masch,backup_schedule:"manual"}};
  }
  if(masch){
    const infos=[`${masch.device_name||"PC Backup Vault"} ist gekoppelt, wurde aber noch nicht für einen Telemetrielauf gestartet`,`kein täglicher Lauf erwartet`];
    if(mAge!==null)infos.splice(1,0,`zuletzt gesehen vor ${tageText(mAge)}`);
    return{...b2,status:"not_configured",health:null,capacityLabel:"Gekoppelt · noch keine Telemetrie",detail:infos.join(" · "),metrics:{...b2.metrics,backup_age_hours:null,telemetry_age_hours:null,machine_last_seen_at:masch?.last_seen_at??null,machine_paired:true,backup_schedule:"manual"}};
  }
  return b2;
}
Deno.serve(async(req:Request)=>{const trace={traceparent:req.headers.get("traceparent")||"",tracestate:req.headers.get("tracestate")||"",baggage:req.headers.get("baggage")||""};if(req.method==="OPTIONS")return new Response(null,{status:204,headers:CORS});if(req.method!=="GET")return new Response(JSON.stringify({error:"method_not_allowed"}),{status:405,headers:CORS});const started=Date.now(),u=new URL(req.url),own=Deno.env.get("SUPABASE_URL")!,service=Deno.env.get("SUPABASE_SERVICE_ROLE_KEY")!;try{if(u.searchParams.get("probe")==="1")return new Response(JSON.stringify({probe:true}),{headers:CORS});if(u.searchParams.get("leitstand")==="1"){const who=await callerRole(req,own,service);if(!who.role)return new Response(JSON.stringify({error:"leitstand_gesperrt",reason:who.reason,hint:"Anmeldung mit einem in kc_system_check_operators freigeschalteten Konto erforderlich."}),{status:403,headers:CORS});const data=await leitstand(own);if(who.role!=="superadmin"){data.sales=[];data.sales_hidden=true}data.viewer={role:who.role};return new Response(JSON.stringify(data),{headers:CORS});}if(u.searchParams.get("history")==="1")return new Response(JSON.stringify({version:"0.4.1-live",history:await history(own),usage:await usage(own)}),{headers:CORS});const H={apikey:service,Authorization:`Bearer ${service}`};let requests=5;const zugaenge:any=await credentials(own,service);requests++;const gitCfg=zugaenge.github_repo,futureCfg=zugaenge.future_academy;const coreP=timed(`${own}/rest/v1/kc_system_check_history?select=checked_at&order=checked_at.desc&limit=1`,{headers:H},trace).catch(()=>null),snapP=timed(`${own}/rest/v1/rpc/kc_system_check_snapshot`,{method:"POST",headers:{...H,"Content-Type":"application/json"},body:"{}"},trace).catch(()=>null),futureP=futureCfg?.endpoint?timed(`${futureCfg.endpoint}/rest/v1/rpc/kc_system_check_public_snapshot`,{method:"POST",headers:{apikey:futureCfg.secret,Authorization:`Bearer ${futureCfg.secret}`,"Content-Type":"application/json"},body:"{}"},trace).catch(()=>null):Promise.resolve(null),gitP=gitCfg?.endpoint?timed(gitCfg.endpoint,{headers:{Accept:"application/vnd.github+json","User-Agent":"KC-System-Check",...(gitCfg.secret?{Authorization:`Bearer ${gitCfg.secret}`}:{})}},trace).catch(()=>null):Promise.resolve(null),teleP=timed(`${own}/rest/v1/kc_backup_machine_telemetry?select=measured_at,updated_at,last_backup_at,last_backup_status,last_backup_original_bytes,last_backup_stored_bytes,storage_target,b2_status,neon_status,status,storage_targets,integrity_result,restore_result,app_version&order=measured_at.desc&limit=1`,{headers:H},trace).catch(()=>null),kiccP=timed(`${own}/rest/v1/kicc_backup_telemetry?select=measured_at,updated_at,last_backup_at,last_backup_status,last_backup_bytes,last_backup_stored_bytes,backup_target,storage_targets,details,integrity_status,last_verify_result,last_restore_test_result,status,app_version&order=measured_at.desc&limit=1`,{headers:H},trace).catch(()=>null),maschP=timed(`${own}/rest/v1/kc_communication_machine_clients?select=device_name,status,approved_at,last_seen_at&source_program=eq.pc-backup-vault&status=eq.active&order=last_seen_at.desc.nullslast&limit=1`,{headers:H},trace).catch(()=>null);const [core,sr,fr,gh,teleReq,maschReq,kiccReq]=await Promise.all([coreP,snapP,futureP,gitP,teleP,maschP,kiccP]);requests+=2;
...
const coreBytes=Number(snap?.database_bytes??0);let fd:any={},gd:any={},tele:any=null;if(fr?.r?.ok)try{fd=await fr.r.json()}catch{}if(gh?.r?.ok)try{gd=await gh.r.json()}catch{}if(teleReq?.r?.ok)try{tele=(await teleReq.r.json())?.[0]||null}catch{}
if(tele){
  tele.last_backup_bytes=tele.last_backup_original_bytes;
  tele.integrity=tele.integrity_result||null;
  tele.restore=tele.restore_result||null;
}
let masch:any=null;if(maschReq?.r?.ok)try{masch=(await maschReq.r.json())?.[0]||null}catch{}
...
if(kicc&&(!tele||Date.parse(kicc.measured_at||"")>Date.parse(tele.measured_at||""))){
  ...
  tele={measured_at:kicc.measured_at,updated_at:kicc.updated_at,last_backup_at:kicc.last_backup_at,last_backup_status:st,last_backup_bytes:kicc.last_backup_bytes,last_backup_stored_bytes:kicc.last_backup_stored_bytes,storage_target:kicc.backup_target,b2_status:/b2|backblaze/.test(ziel)||!ziel?st:null,neon_status:/neon/.test(ziel)?st:null,integrity:kicc.integrity_status||kicc.last_verify_result||null,restore:kicc.last_restore_test_result||null,app_version:kicc.app_version||null,storage_targets:kicc.storage_targets||[],details:kicc.details||{},quelle:"kicc_backup_telemetry"};
}const [neonDirect,b2Direct,r2Direct,ociDirect,repoConfig,cloudflareEdgeResult]=await Promise.all([optionalEndpoint("NEON"),optionalEndpoint("B2"),optionalEndpoint("R2"),optionalEndpoint("OCI"),publicConfig(gd),cloudflareEdge(own,service,trace)]);requests++;for(const x of[neonDirect,b2Direct,r2Direct,ociDirect])if(x)requests++;const futureBytes=Number(fd?.database_bytes??0),cu=pct(coreBytes,FREE_DB_BYTES),coreOk=!!core?.r?.ok,coreMs=core?.ms??null,coreStatus=coreOk?(coreMs!==null&&coreMs>3000?"warning":"healthy"):"critical",coreHealth=coreOk?(coreMs!==null&&coreMs>3000?82:coreMs!==null&&coreMs>1500?90:100):35;
...
const deepNeonCheck=u.searchParams.get("deep_neon")==="1"||u.searchParams.get("trigger")==="manual";
if(neonCred&&deepNeonCheck){const [nq,bq]=await Promise.all([neonQuery(neonCred),backupQuery(neonCred)]);requests+=2;neon=neonResult(nq,neon);sicherung=backupResult(bq)}
else if(neonCred){sicherung={...sicherung,status:"unknown",health:null,capacityLabel:"Tiefenprüfung nicht ausgeführt",detail:"Automatischer Lauf weckt Neon nicht auf; Backup-Tiefenprüfung erfolgt bei manuellem/deep_neon Lauf",metrics:{deep_check:false,compute_wakeup_avoided:true}};neon={...neon,metrics:{...(neon.metrics||{}),deep_check:false,compute_wakeup_avoided:true}}}
let b2=endpointResult("b2","Backblaze B2","storage",b2Direct,"Adapter bereit; noch keine B2-Telemetrie oder sichere Read-only-Verbindung");const b2Target=Array.isArray(tele?.storage_targets)?tele.storage_targets.find((x:any)=>x?.id==="b2_backup"):null;if(!b2Direct&&b2Target){const t=telemetryState(b2Target.status);b2={...b2,status:t.status,health:t.health,latency:Number.isFinite(Number(b2Target.latencyMs))?Number(b2Target.latencyMs):null,capacityLabel:tele.last_backup_bytes?`${mb(Number(tele.last_backup_bytes))} MB Original${tele.last_backup_stored_bytes!==null&&tele.last_backup_stored_bytes!==undefined?` · ${mb(Number(tele.last_backup_stored_bytes))} MB gespeichert`:""}`:"PC Backup Vault B2-Prüfung",detail:`${b2Target.detail||"B2-Ziel geprüft"}${Number.isFinite(Number(b2Target.objectCount))?` · ${Number(b2Target.objectCount)} Objekt(e) im Vault-Präfix`:""}${Number.isFinite(Number(b2Target.storedBytes))?` · ${mb(Number(b2Target.storedBytes))} MB gesamt gespeichert`:""}${b2Target.checkedAt?` · geprüft ${b2Target.checkedAt}`:""} · Backup Vault wird manuell gestartet`,metrics:{direct_check:false,telemetry:true,target_health:true,measured_at:tele.measured_at,target_checked_at:b2Target.checkedAt||null,last_backup_at:tele.last_backup_at,last_backup_status:tele.last_backup_status,last_backup_bytes:tele.last_backup_bytes,last_backup_stored_bytes:tele.last_backup_stored_bytes,prefix_object_count:Number.isFinite(Number(b2Target.objectCount))?Number(b2Target.objectCount):null,prefix_stored_bytes:Number.isFinite(Number(b2Target.storedBytes))?Number(b2Target.storedBytes):null,storage_target:tele.storage_target}}}else if(!b2Direct&&tele?.b2_status){const t=telemetryState(tele.b2_status);b2={...b2,status:t.status,health:t.health,capacityLabel:tele.last_backup_bytes?`${mb(Number(tele.last_backup_bytes))} MB Original${tele.last_backup_stored_bytes!==null&&tele.last_backup_stored_bytes!==undefined?` · ${mb(Number(tele.last_backup_stored_bytes))} MB gespeichert`:""}`:"PC Backup Vault Telemetrie",detail:`B2 ${tele.b2_status} · letzter Backup-Status ${tele.last_backup_status||"unbekannt"}${tele.integrity?` · Integrität ${tele.integrity}`:""}${tele.restore?` · Restore-Test ${tele.restore}`:""}${tele.app_version?` · App v${tele.app_version}`:""} · Backup Vault wird manuell gestartet`,metrics:{direct_check:false,telemetry:true,measured_at:tele.measured_at,last_backup_at:tele.last_backup_at,last_backup_status:tele.last_backup_status,last_backup_bytes:tele.last_backup_bytes,last_backup_stored_bytes:tele.last_backup_stored_bytes,storage_target:tele.storage_target}}}
if(!b2Direct){b2=backupAgeRules(b2,tele,masch)}
const runtime=tele?.details?.runtime;
if(runtime&&typeof runtime==="object"){
  const done=Number(runtime.bytes_done),total=Number(runtime.bytes_total),speed=Number(runtime.speed_bps),eta=Number(runtime.eta_seconds),percent=Number(runtime.percent);
  const phase=String(runtime.phase||"").slice(0,80),target=String(runtime.target||"").slice(0,80);
  const runtimeParts:string[]=[];
  if(Number.isFinite(percent))runtimeParts.push(`${Math.max(0,Math.min(100,percent))} %`);
  if(Number.isFinite(speed)&&speed>=0)runtimeParts.push(`${mb(speed)} MB/s`);
  if(Number.isFinite(eta)&&eta>=0)runtimeParts.push(`ETA ${Math.round(eta)} s`);
  if(phase)runtimeParts.push(phase);
  b2={...b2,capacityLabel:runtimeParts.length?`Backup läuft · ${runtimeParts.join(" · ")}`:b2.capacityLabel,detail:`${b2.detail} · Live-Fortschritt aus Backup Vault`,metrics:{...b2.metrics,runtime:{bytes_done:Number.isFinite(done)?done:null,bytes_total:Number.isFinite(total)?total:null,speed_bps:Number.isFinite(speed)?speed:null,percent:Number.isFinite(percent)?percent:null,eta_seconds:Number.isFinite(eta)?eta:null,phase:phase||null,target:target||null}}};
}
