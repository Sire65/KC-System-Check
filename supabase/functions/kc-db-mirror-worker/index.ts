...
Deno.serve(async(req)=>{
  ...
  try{
    for(let i=0;i<requested.length;i++){
      ...
      try{
        ...
        const v=await neon.unsafe(verifySql);const tc=String(v[0]?.row_count??"0"),th=String(v[0]?.content_hash??""),ok=sc===tc&&sh===th,endMs=Date.now(),durationMs=Math.max(1,endMs-startMs),bps=Math.round(bytes/(durationMs/1000));
  ...
  }finally{await neon.end({timeout:5})}
