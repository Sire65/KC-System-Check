...
test('manager and market cash register remain prepared until current versions are connected',()=>{const s=fs.readFileSync('js/leitstand.js','utf8');assert.match(s,/Anbindung vorbereitet · aktuelle Programmversion später anbinden/);assert.match(s,/Heartbeat-Anbindung in der aktuellen Marktkasse später ergänzen/);assert.match(s,/Heartbeat-Anbindung in der aktuellen Manager-Version später ergänzen/)});
...
test('live UI renders backup health without inventing green status',()=>{const s=fs.readFileSync('js/leitstand.js','utf8');assert.match(s,/renderBackup/);assert.match(s,/Noch keine Messwerte vorhanden/);assert.match(s,/B2\/Neon werden nicht künstlich grün angezeigt/)});
